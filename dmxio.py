...
class DmxConnection(object):
    ...
    HEADER = (b'A', b'r', b't', b'-',  
              b'N', b'e', b't', b'\x00', 
              b'\x00', b'\x50', 
              b'\x00', b'\x0e', # Version 14
              b'\x00', b'\x00', 
              b'\x00', b'\x00',  # Universe 0 (Only supportes 1 universe)
              b'\x02', b'\x00') # 512 Channels
    ...
    def set_channels(self, start_channel, values):
        ...
        """
        if not ((1 <= start_channel <= DMX_SIZE) and (start_channel + len(values) <= DMX_SIZE+1)):
